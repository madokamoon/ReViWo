...
def delete_oldest_file(directory):  
    oldest_file = find_oldest_file(directory)  
    if oldest_file:  
        try: 
            ...
            print(f"Error deleting file {oldest_file}: {e.strerror}")  
    else:  
...
class TensorBoardOutputHandler(KVWriter):
    ...
    def writekvs(self, kvs: Dict) -> None:
        ...
        def summary_rgb(k, v):
            if v.shape[-1] == 3:
                v = v.transpose(2, 0, 1)
            kwargs = {'tag': k, 'img_tensor': v, 'global_step': self.step, 'dataformats': "CHW"}
            self.writer.add_image(**kwargs)
...
class Logger(object):
    ...
    def _init_dirs(self) -> None:
        ...
        self._model_dir = os.path.join(self._dir, "best_model")
    ...
    def save_model(self, save_func, max_ckpt, **kwargs):
        '''
        save_func: the save-model function; \\
        max_ckpt: the limit number of the possible checkpoint; \\
        **kwargs: must include the needed arg name of the save_func, and the value is the file's name, e.g. checkpoint_file="tokenizer/epoch_100.pth". 
        '''
        save_dir = None
        for k, v in kwargs.items():
            if k in ["save_checkpoint_file", "checkpoint_file", "save_dir"]:
                kwargs[k] = str(os.path.join(self.checkpoint_dir, v))
                save_dir = str(os.path.dirname(kwargs[k]))
                
        save_func(**kwargs)
                
        if isinstance(max_ckpt, int) and max_ckpt > 0:
            if count_files_in_directory(save_dir) > max_ckpt:
                delete_oldest_file(save_dir)
        
    def load_model(self, load_func, **kwargs):
        '''
        load_func: the load-model function; \\
        **kwargs: must include the needed arg name of the load_func, and the value is the file's name, e.g. checkpoint_file="tokenizer/epoch_100.pth".
        '''
        for k, v in kwargs.items():
            if k in ["load_checkpoint_file", "checkpoint_file", "load_dir"]:
                kwargs[k] = self.checkpoint_dir + v
...
def make_log_dirs(
    ...
    exp_name = f"timestamp_{timestamp}"
    log_dirs = os.path.join(project_dir + "/logs", task_name, exp_name)
    if dir_base is not None:
        log_dirs = os.path.join(dir_base, task_name, exp_name)
