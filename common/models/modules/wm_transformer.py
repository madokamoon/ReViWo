...
class WMTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.x_embedder = nn.Embedding(config.vocab_size, config.n_embd)
        self.a_embedder = nn.Linear(config.action_dim, config.n_embd)
    ...
    def forward(self, x, a):
        ...
        a = a.unsqueeze(dim=-2).repeat(1, 1, self.config.n_tokens_per_frame, 1).reshape(b, t, c)
