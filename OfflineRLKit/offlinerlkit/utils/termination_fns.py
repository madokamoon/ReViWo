...
def terminaltion_fn_metaworld_with_mvembedding(obs, act, next_obs):
    return np.array([False] * obs.shape[0])[:, None]        
...
def get_termination_fn(task):
    ...
    elif task in list(ALL_V2_ENVIRONMENTS.keys()) or "Panda" in task:
        return terminaltion_fn_metaworld_with_mvembedding
    else:
        raise np.zeros
