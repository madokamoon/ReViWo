...
class COMBOPolicy(CQLPolicy):
    ...
    def rollout(
        ...
        return rollout_transitions, \
            {"num_transitions": num_transitions, "reward_mean": rewards_arr.mean(), "obs_min": rollout_transitions['next_obss'].min(), "obs_max": rollout_transitions['next_obss'].max()}
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self.max_norm is not None:
            nn.utils.clip_grad.clip_grad_norm_(parameters=self.actor.parameters(), max_norm=self.max_norm)
        ...
        if self.max_norm is not None:
            nn.utils.clip_grad.clip_grad_norm_(parameters=self.critic1.parameters(), max_norm=self.max_norm)
        self.critic1_optim.step()

        self.critic2_optim.zero_grad()
        critic2_loss.backward()
        if self.max_norm is not None:
            nn.utils.clip_grad.clip_grad_norm_(parameters=self.critic2.parameters(), max_norm=self.max_norm)
