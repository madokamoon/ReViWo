...
class EnsembleDynamics(BaseDynamics):
    ...
    def step(
        ...
        if self.step_clip:
            next_obs = np.clip(next_obs, a_min=self.obs_min, a_max=self.obs_max)
            reward = np.clip(reward, a_min=self.reward_min, a_max=self.reward_max)
    ...
    def learn(
        ...
        for batch_num in trange(int(np.ceil(train_size / batch_size)), leave=False, ncols=130):
